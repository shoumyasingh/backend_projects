...
class Student:
    ...
    def add_student(self, student_id, first_name, last_name):
        self.__data.add__student(self, student_id, first_name, last_name)

    def read_student(self, student_id):
        return self.__data.read__student(self, student_id)
...
class Policy:
    ...
    def set_semester(self):
        ...
        no_of_assignments = int(input("Enter number of programming assignments from 0-6"))
        while no_of_assignments < 0 or no_of_assignments > 6:
            print("Enter valid range (0-6)")
            no_of_assignments = int(input("Enter number of programming assignments from 0-6"))
        self.a_num = no_of_assignments
        self.policy['Assignment Total Number'] = no_of_assignments

        no_of_tests = int(input("Enter number of tests between (0-4): "))
        while no_of_tests < 0 or no_of_tests > 4:
            print("Enter valid range between (0-4): ")
            no_of_tests = int(input("Enter number of tests between (0-4): "))
        self.t_num = no_of_tests
        self.policy['Test Num'] = no_of_tests

        no_of_final_exam = int(input("Enter number of Final exams between (0-1): "))
        while no_of_final_exam < 0 or no_of_final_exam > 1:
            print("Enter valid range between (0-1)")
            no_of_final_exam = int(input("Enter number of Final exams in (0-1): "))
        self.f_num = no_of_final_exam
        self.policy['Final Exam Numbers'] = no_of_final_exam

        print("Enter the weights for Test, Assignment, and Final exam: ")
        print("Total weights must add to 100% for the semester")

        assignment_wt = float(input("Please enter the weight percentage of total programming assignments: "))
        self.a_wt = assignment_wt
        self.policy['Assignment Weight'] = assignment_wt

        test_wt = float(input("Please enter the weight percentage of all the tests: "))
        self.t_wt = test_wt
        self.policy['Test Weight'] = test_wt

        final_exam_wt = float(input("Please enter the weight percentage for the final exams: "))
        self.f_wt = final_exam_wt
        self.policy['Final Exam Total Weight: '] = final_exam_wt

    def write_policy(self):
        self.__data.write__policy(self, self.__a_num, self.__t_num, self.__f_num, self.__a_wt, self.__t_wt, self.__f_wt)

    def read_policy(self):
        return self.__data.read__policy(self)
...
class Score:
    ...
    def record_score(self, change_score, number, student_id, type_of_score):
        if type_of_score == "A":
            ...
            print("Unsupported type of score, so enter the correct score.")
    ...
    def calculate_final_score(self):
        ...
        print("Successfully committed the student final grades to Grades file")


def main():
    std = Student(100, "s", "s")
    std.add_student(111, "shoumya", "singh")
    app = Policy()
    app.set_semester()
    app.write_policy()
